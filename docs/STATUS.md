...
A nonempty intersection is a measured `true` and lists the intersecting nodes. An empty intersection is a measured `false`. A program that asserts no obligation, or whose obligations have no flow that names Weavatrix nodes, is `unmeasured` with an explicit reason; it never inherits the repository-wide `graph_diff` bit. `join_triangle` treats unmeasured code as not-changed for classification and emits no `WVQ-BEHAV-*` finding from that cell. The artifact records `code_unmeasured_programs`; that list makes the axis `unmeasured` rather than clean.
...
1. **Privacy-safe request identity.** Method + path + content type + canonical body hash, with GraphQL keyed by operation name, query hash, and variables hash. No raw bodies enter the profile.
2. **Honest visual evidence.** Rename the misleading `Pixel` axis, attach a real content digest, and only then build an actual perceptual comparison engine as a separate layer.
3. **Split `crates/wvq-command-bus/src/service.rs`.** It is past 12 000 lines; this is engineering risk, not style.
4. **Bounded failure evidence.** On a Playwright failure, assemble a diagnostic reel — frame before the action, the highlighted semantic target, frame after, and the assertion or geometric cause — behind a CAS handle. Diagnostic only: it must never become a verdict source and must not run on the green path.
5. **Snapshot reach.** Shadow DOM, same-origin iframes, the effective clip chain, and responsive sentinels.
6. **Application Surface Graph → Coverage Autopilot.**
7. **Then breadth.** Continuous recorder, extended cassette (WebSocket, cookie/storage replay, schema patching), Studio frontend, `wvq init`, `wvq baseline` with `OBSERVED_ONLY`, and first-class hover/scroll/drag-drop/upload/download/popup/tab actions.
8. **Advanced producers.** Project metamorphic adapters and the browser-feedback exploration loop.
