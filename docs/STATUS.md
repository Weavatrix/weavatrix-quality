...
Session: scoped OpenSpec authorization
...
| Scoped `CodeDelta` | ✅ | ❌ | ❌ change-wide bool | ❌ no flow intersection |
...
`wvq-proof` turns that scope into a per-program decision. A program is spec-authorized only when *every* obligation it asserts lies inside the changed scope; one matching obligation cannot authorize a mixed program, and a program that asserts no obligation is never authorized. The `delta-triangle` artifact is now `schema_v: 2` and records `spec_authorized`, `authorized_obligations`, and `unauthorized_obligations` per program; v1 documents stay readable.
...
`CodeDelta` has no equivalent scoping yet. It remains one repository-wide boolean derived from `graph_diff`, so any code change anywhere still satisfies the code axis for every program. That is the next correctness step, not a solved one.
...
1. **Scoped `CodeDelta`.** Replace the repository-wide boolean with the intersection of program → obligation → protected flow → Weavatrix-changed nodes. Where that intersection cannot be measured, the axis must report `unmeasured` rather than inheriting a global `true`.
2. **Privacy-safe request identity.** Method + path + content type + canonical body hash, with GraphQL keyed by operation name, query hash, and variables hash. No raw bodies enter the profile.
3. **Honest visual evidence.** Rename the misleading `Pixel` axis, attach a real content digest, and only then build an actual perceptual comparison engine as a separate layer.
4. **Split `crates/wvq-command-bus/src/service.rs`.** It is past 12 000 lines; this is engineering risk, not style.
5. **Bounded failure evidence.** On a Playwright failure, assemble a diagnostic reel — frame before the action, the highlighted semantic target, frame after, and the assertion or geometric cause — behind a CAS handle. Diagnostic only: it must never become a verdict source and must not run on the green path.
6. **Snapshot reach.** Shadow DOM, same-origin iframes, the effective clip chain, and responsive sentinels.
7. **Application Surface Graph → Coverage Autopilot.**
8. **Then breadth.** Continuous recorder, extended cassette (WebSocket, cookie/storage replay, schema patching), Studio frontend, `wvq init`, `wvq baseline` with `OBSERVED_ONLY`, and first-class hover/scroll/drag-drop/upload/download/popup/tab actions.
9. **Advanced producers.** Project metamorphic adapters and the browser-feedback exploration loop.
