...
Session: P1 — Surface Evidence Matrix as a read-only MCP/Studio view
...
`ApplicationSurfaceGraph` is a live run artifact (`application-surface-graph`) and a read-only MCP/Studio projection (`protected` / `partial` / `unmeasured`). The Surface Evidence Matrix is the next read-only artifact (`surface-evidence-matrix`): one row per named surface with `intent | runtime | test | proof | protection | UI | a11y | mutation` cells of `present` / `absent` / `unmeasured`. Intent and test are closed-world from declared bindings; protection uses the Coverage Autopilot four-way split; runtime, proof, UI, a11y, and mutation stay unmeasured until those producers join. A missing column is unmeasured, never a fake absent. Truncated binding reach marks touched surfaces unmeasured, not present. Neither view is a gate: `quality_verify` still composes from the existing axes. Unknown schema versions fail closed. Studio copies the matrix onto the change summary.
...
1. **Gap classification + cheapest-evidence planner, then observe-only calibration.** Coverage Autopilot closed-loop generation waits until those exist. The Surface Evidence Matrix is now a read-only persisted projection and Autopilot input, not a gate and not a planner.
