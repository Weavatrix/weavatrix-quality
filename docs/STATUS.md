...
Session: breadth — continuous recorder package
...
The optional `@wvq/recorder` package and `wvq ingest-journal` admit a fail-closed continuous observation journal as `OBSERVED_ONLY` BehaviorGraph evidence. Unknown schema, `observed_only: false`, XPath, path-shaped session ids, actions other than navigate/activate/fill/select/press, more than 1000 events, and documents over 1 MiB fail closed. Fill and select must name journal `data` keys. Useful journals persist new states and edges plus CAS traces; duplicates are discarded as `no_new_behavior`. The journal never claims obligations or APIs, never creates a recorded session, never previews or promotes, and cannot seal. It does not fill SurfaceGraph or the candidate corpus. `ingest-journal` is CLI/npm-only — not a default MCP tool. This is not extended cassette, not Studio frontend, not `wvq baseline`, and not Coverage Autopilot.
...
1. **Then breadth.** Extended cassette, Studio frontend, and `wvq baseline` with `OBSERVED_ONLY`. `wvq init`, first-class browser actions, and the continuous recorder package are already on `main`. Coverage Autopilot closed-loop generation still waits. Bounded failure evidence (`failure_reel`) stays diagnostic-only.
