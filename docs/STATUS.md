...
Session: P1 — exact product test bindings
...
P0 from the 2026-09-02 reaudit is implemented on this tree. `wvq doctor`, product OpenSpec `wvq-invariants`, Studio exception cards, and exact product test bindings are on `main`. Remaining P1 is a bounded cargo-test filter so `wvq run`/`verify --observe-only` can dogfood those cases without widening to the whole workspace, and the 30–50 PR observe-only campaign.
