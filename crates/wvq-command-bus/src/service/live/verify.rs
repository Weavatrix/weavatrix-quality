...
use super::super::verify_reply::stored_range;
...
impl LiveService {
    ...
    pub(in crate::service) fn verify(&self, cmd: &VerifyCommand) -> Result<VerifyReply, BusError> {
        ...
        reply.behavior_surface = match &run {
            Some(run) => super::super::persist_surface::load_behavior_surface(&store, &run.id)?,
            None => BehaviorSurfaceView::absent(),
        };
