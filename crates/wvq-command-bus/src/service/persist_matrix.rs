...
pub(in crate::service) fn surface_evidence_from(
    ...
    let columns = SurfaceEvidenceColumns {
        ...
        runtime: runtime_column(&surface_graph, sources.browser_runs),
        proof: proof_column(&surface_graph, sources.graph, sources.bindings, sources.records),
...
fn surfaces_for_request(graph: &ApplicationSurfaceGraph, method: &str, url: &str) -> BTreeSet<String> {
...
fn proof_column(
    ...
    for record in records.iter().filter(|record| record.passed) {
        ...
        {
            let Ok(normalized) = serde_json::from_slice::<NormalizedTestRun>(&artifact.bytes) else {
                continue;
            };
            for binding in bindings {
                ...
                if !normalized.cases.iter().any(|item| {
                    item.name == case
                        && item.status == wvq_runtime::TestStatus::Pass
                }) {
                    continue;
                }
                saw_exact = true;
