...
use wvq_mcp::{HostProfile, parse_host_args, quality_server, recovery_server, runtime_config};

fn main() -> ExitCode {
    let args = env::args().skip(1).collect::<Vec<_>>();
    if args.iter().any(|arg| arg == "--help" || arg == "-h") {
        println!(
            "wvq-mcp — Weavatrix Quality MCP\n\nUsage:\n  wvq-mcp [--repo PATH] [--profile default|recovery] [--change ID] [--base REF] [--head REF|WORKTREE]"
        );
