...
The same browser run also produces live Delta Triangle evidence by replaying the exact head-selected program, seed, and sealed oracles against the merge-base runtime. Git decides whether the change modified its OpenSpec authority, Weavatrix decides whether code nodes or edges changed, and Playwright supplies the behavior axis. Preview-server origins are excluded from network identity, every observation is compared in order, and an incomplete replay is `unmeasured` rather than clean. A measured code + behavior change with no OpenSpec delta emits `WVQ-BEHAV-001` and blocks the ordinary composite verdict even if the sealed assertion still passes.
