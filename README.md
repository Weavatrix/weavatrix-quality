...
The Surface Evidence Matrix is a read-only artifact, not a score. Intent comes from OpenSpec obligations mapped onto production code; a test binding is not product intent. Coverage is normalized hits on Weavatrix nodes; Protection Continuity comes from the revision snapshot. Runtime, Proof, UI, A11y, and Mutation fill only when those producers actually ran. Missing producers stay `unmeasured`. v1 artifacts that stored coverage under `protection` are migrated on read. The cheapest-evidence planner will not recommend BrowserExplore until that loop exists, offers source mutation for mutation gaps, and routes intent gaps to spec recovery / product review rather than inventing a test. Unmeasured cells get a measurement plan; measured-absent cells get an evidence-gap plan.

Behavior surfaces hang off the Application Surface Graph. A Role/State/Action/Flag combination exists only when it was declared, observed, recorded, story-backed, or graph-reachable. Two facts are never crossed into a third combination: recording `admin` and `empty_cart` separately does not invent `admin × empty_cart`. A live Playwright action on `/checkout` and a journal `admin` on the same route stay two facts. Live runs feed Observed route+action spans; they never invent a role, flag, or endpoint from `activate`. Unknown surfaces and surface-only facts are dropped. The projection truncates at 512 and is not a gate.
...
Mutation hints now have a real source-execution path. For changed non-test TS/JS, Go, and Rust lines, WVQ plans only the bounded built-in catalogue, creates a detached worktree at the exact head commit, overlays the requested working-tree content when needed, applies one reversible edit, and runs only exact obligation-bound Go, Vitest/Storybook-Vitest, or cargo-test cases. Ownership is the same `ObligationCodeSurface` used by `CodeDelta`: exact coverage, measured traces, directed Weavatrix test→production reach, protection flows, then explicit mappings. A test-file binding does not claim production code by itself; Weavatrix must reach the mutated file. Missing owners stay `unmeasured` — WVQ does not fall back to every candidate obligation. The user's checkout is never edited. A compile failure, missing report, ambiguous case identity, unsupported runner, or spent execution ceiling is `invalid`/`unmeasured`, never a killed mutant. Results are attributed per obligation: a test for one obligation cannot strengthen another. A surviving, invalid, or required-but-missing measurement turns an otherwise `PROVEN` proof into `PARTIAL`; forged counters, policy mismatches, and unknown artifact states fail closed. The producer caps one mutant at 120 seconds, the whole mutation phase at 600 seconds, source edits at 32, obligation-case decisions at 128, and output at 2 MiB per process. This path uses zero model tokens.

The built-in source catalogue is `boundary_flip`, `equality_flip`, `bool_flip`, `logical_flip`, `off_by_one`, `remove_branch`, `remove_sort`, `wrong_permission`, `omit_callback`, `omit_error`, and `collection_boundary` for TS/JS; `err_nil_flip`, `boundary_flip`, `return_zero`, `skip_branch`, `ignore_context`, and `invert_bool` for Go; and `boundary_flip`, `invert_bool`, `is_ok_err_flip`, `is_some_none_flip`, and `eq_neq_flip` for Rust. Rust mutants are judged by an exact `cargo-test` case, not JUnit. Project-semantic hints such as `omit_group` are not guessed onto source syntax; they remain visible in the mutation artifact as unmapped limitations.
...
`wvq init` writes a fail-closed `.weavatrix-quality/config.yaml`. This repository commits that policy and a product OpenSpec change `wvq-invariants` so `wvq spec validate --change wvq-invariants` and `wvq doctor` work here. Each invariant is bound to one existing exact Cargo case. A green suite without that mapping remains `UNPROVEN`. An impacted `wvq run` sends each bound case as a cargo TESTNAME (`cargo test --workspace --all-targets <case>`), one process per case, instead of widening to the whole workspace. `wvq doctor` is read-only discovery: it names detected ecosystems, registered runners, existing OpenSpec change folders, policy loadability, and already-declared bindings. Detection is not authority — doctor never writes, never seals, and never invents a binding or an OpenSpec change. It is CLI-only, not a default MCP tool. `wvq ingest-journal` admits a privacy-safe continuous observation journal as `OBSERVED_ONLY` BehaviorGraph evidence. Stored journals fill the Surface Evidence Matrix Runtime column on the next `wvq run` (route/component/navigate). They never set Intent, Test, or Proof. `wvq ingest-cassette` admits a HAR as a privacy-safe network profile (replay stays off). `wvq baseline` snapshots existing debt fingerprints as `OBSERVED_ONLY`; `--decision` must be `observed_only` and there is no `accept_all`. `wvq verify --observe-only` keeps the composite verdict but exits 0 for Stage A calibration.

Unknown and duplicate flags fail instead of being silently ignored. A blocking `CONTRADICTED` verify verdict exits with code 2; unresolved evidence exits with code 1. Observe-only never changes that classification; it only changes the process exit for CI. This repository's Linux CI runs `wvq spec validate`, `wvq doctor`, and `wvq verify --observe-only` against `wvq-invariants` after a green workspace build. Spec and doctor are blocking. Observe-only records `UNPROVEN` without failing the job and posts GitHub notices.

`qualityd` serves the exception-first Studio cockpit on local HTTP. Each unresolved proof is an exception card: intent, matched surface, protection/proof/runtime/UI/a11y/mutation cells, cheapest next evidence, and source candidates. A neighbouring surface is not a join; missing cells stay `unmeasured`. The Application Surface Graph, Behavior Surface Graph, Surface Evidence Matrix, and cheapest-evidence plan are read-only artifacts on that cockpit and on MCP — they are not a second policy layer.
