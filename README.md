...
The Surface Evidence Matrix is a read-only artifact, not a score. Intent comes from OpenSpec obligations mapped onto production code; a test binding is not product intent. Coverage is normalized hits on Weavatrix nodes; it is not Protection Continuity. Missing producers stay `unmeasured`. v1 artifacts that stored coverage under `protection` are migrated on read.
