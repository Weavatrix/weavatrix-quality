...
export function predicateTarget(predicate) {
    ...
    if ("predicate" in value)
        return predicateTarget(value.predicate);
    return undefined;
}
/**
 ...
 */
export async function captureFailureReelFrames(page, request) {
    const limitations = [];
...
export async function highlightLocator(locator) {
...
export async function clearHighlight(locator) {
    ...
    await handle.evaluate((node) => {
        const el = node;
        if (!el.hasAttribute("data-wvq-failure-highlight"))
            return;
...
function reelPath(request, slot) {
    return join(request.evidence_dir, `${safeName(request.program_id)}-reel-${request.step}-${slot}.png`);
}
