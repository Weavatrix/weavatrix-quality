...
export async function installSemanticRecorder(context, page, hooks, config) {
    assertRecorderConfig(config);
    await page.exposeBinding("__wvqRecordEvent", async (_source, capture) => {
...
export function assertRecorderCapture(value) {
...
function assertRecordedAction(action) {
    if (!action || typeof action !== "object")
        throw new Error("recorded action is malformed");
...
function pageRecorderInit(config) {
    if (window.__wvqRecorderInstalled)
        return;
    ...
    const targetOf = (element, includeScope = true) => {
        const target = {};
        ...
        if (role)
            target.role = role;
        if (name)
            target.accessible_name = name;
        if (component)
            target.component_hint = component;
        if (!testId && !role && !name && !component && id)
            target.fallback_css = `#${cssEscape(id)}`;
        if (includeScope) {
            const scope = element.parentElement?.closest(`[role="dialog"],dialog,[${testIdAttribute}],[data-component],tr,li`);
            const semanticScope = scope && scope !== element ? targetOf(scope, false) : undefined;
            if (semanticScope)
                target.scope = semanticScope;
        }
        return Object.keys(target).length > 0 ? target : undefined;
    };
    const semanticElement = (raw) => {
        if (!(raw instanceof Element))
            return undefined;
        return raw.closest(`button,a[href],input,select,textarea,[role],[${testIdAttribute}],[data-component]`) || undefined;
    };
    const emit = (capture) => {
        ...
        const now = Date.now();
        if (fingerprint === lastFingerprint && now - lastAt < 50)
            return;
    ...
    const fixtureFor = (value) => fixtureEntries.find(([, fixtureValue]) => fixtureValue === value)?.[0];
    document.addEventListener("click", (event) => {
        const element = semanticElement(event.target);
        if (!element || element instanceof HTMLSelectElement || element instanceof HTMLTextAreaElement)
            return;
        ...
        if (target)
            emit({ action: { action: "activate", target } });
    }, true);
